...
function fixture(t, options = {}) {
  ...
  const env = { ...process.env, HOME: home, GH_CONFIG_DIR: path.join(home, '.gh'), PATH: `${bin}:${process.env.PATH}` }
  delete env.ATLAS_TELEGRAM_BOT
  delete env.ATLAS_TELEGRAM_CHAT
  ...
  fs.writeFileSync(path.join(bin, 'curl'), `#!${process.execPath}\nrequire('node:fs').appendFileSync('.atlas/curl-calls', JSON.stringify(process.argv.slice(2)) + '\\n')\n`, { mode: 0o755 })
  ...
  write('.atlas/config.json', { gates: ['test -f implementation.txt'], timeout: 2, retries: options.retries ?? 3 })
  write('.atlas/backlog.md', '# Tasks\n\n## TODO\n\n### T-1: First task\n- **Acceptance:** implementation.txt exists\n\n## IN_PROGRESS\n\n## DONE\n\n## DELAYED\n')
  if (options.git !== false) { git('init', '-b', 'main'); git('add', '.'); git('commit', '-m', 'chore: initial') }
  if (options.remote) {
