...
Each task is a vertical slice delivered as its own reviewable PR: it works and
can be reviewed on its own, may build on an earlier task, must not depend on a
later task, and must not overlap another task's scope. Do not write
verification-only or "check everything" tasks; each task verifies its own
work. Order tasks bottom-to-top, the same order they will be stacked in.
