...
### Fixed
- **CI shellcheck gate**: removed the remaining `SC2015` patterns from cleanup paths so the CI shellcheck step passes with info-level findings enabled

## [3.2.4] - 2026-03-23

### Fixed
- **Shellcheck compliance**: cleaned up bash patterns flagged in CI so `atlas.sh` and `notify-telegram.sh` pass the shellcheck gate introduced in CI

## [3.2.3] - 2026-03-23

### Fixed
- **NPM package metadata**: kept the published package metadata canonical and synchronized the lockfile package version with `package.json` so npm install/publish artifacts stay aligned

## [3.2.2] - 2026-03-23

### Fixed
- **NPM publishing restored**: `release.yml` now uses Node 24 again so npm Trusted Publishing can authenticate correctly during GitHub Actions releases
...
### Changed
- **Critical Rules section** added to CLAUDE.md and AGENTS.md (mandatory version bump, --admin merge flag)
- **GitFlow instructions** translated to English in CLAUDE.md and AGENTS.md
- **docs/ directory** added to .gitignore and cleaned up
