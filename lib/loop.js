...
const GATE_OUTPUT_LIMIT = 4000

function interrupted() {
  return Object.assign(new Error('Interrupted'), { exitCode: 130 })
}
...
async function runGates(cwd, config, log, signal) {
  for (const gate of config.gates) {
    ...
    if (result.code === 130) throw interrupted()
    if (result.code !== 0) return `${gate}\n${result.output.slice(-GATE_OUTPUT_LIMIT)}`
  }
  return null
}
...
async function attempt(cwd, config, task, promptFile, log, signal) {
  ...
  if (result.code === 130 || signal.aborted) throw interrupted()
  if (git.currentBranch(cwd) !== config.branch) throw new Error('The agent switched Git branches; stopping to preserve evidence.')
  if (result.code !== 0) return { error: `Provider exited with code ${result.code}.` }
  const resultFile = `${promptFile.replace(/\.prompt\.md$/, '')}.result.json`
  let report
  try {
...
function prBody(cwd, branch, base) {
...
async function run(cwd, config, iterations, signal) {
  ...
  for (; count < iterations; count++) {
    ...
    for (let attemptNumber = 1; attemptNumber <= config.retries; attemptNumber++) {
      ...
      const outcome = await attempt(cwd, config, task, promptFile, log, signal)
    ...
    git.publish(cwd, branch, base, prBody(cwd, branch, base))
  }
