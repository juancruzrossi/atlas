...
// Args match notify-telegram.sh's contract: ITERATION MAX PROJECT TASK STATUS PENDING.
function notifyTelegram(cwd, iteration, config, taskId, status, pending) {
  ...
  const args = [iteration, config.iterations, path.basename(cwd), taskId, status, pending].map(String)
  return new Promise(resolve => {
    const child = spawn(path.join(atlasHome, 'notify-telegram.sh'), args, { cwd, stdio: 'ignore', timeout: 15000 })
...
async function run(cwd, config, signal) {
  ...
  await notifyTelegram(cwd, count, config, last.id, last.status, pending)
