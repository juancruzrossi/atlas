...
function integer(value, name, min = 0, max = 2147483647) {
...
function status(cwd) {
  const dir = path.join(cwd, '.atlas')
  const counts = backlog.counts(backlog.load(path.join(dir, 'backlog.md')))
  const delayed = backlog.load(path.join(dir, 'backlog.md')).tasks.filter(t => t.state === 'DELAYED')
  let branch = null
  try {
