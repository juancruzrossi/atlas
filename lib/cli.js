...
const DEFAULT_CONFIG = { provider: 'claudecode', gates: [], iterations: 25, timeout: 3600, retries: 3 }
...
Commands
  init                Set up .atlas/ in this project
  plan "<feature>"    Turn a feature into backlog tasks
  [run] [N]           Work through up to N tasks (default 25)
  status [--json]     Show progress
  help                Show this help

Options
  --cli <provider>    claudecode | codex | opencode (remembered)
...
function loadConfig(cwd, options) {
  ...
  return {
    ...
    iterations: options.iterations ?? integer(saved.iterations ?? DEFAULT_CONFIG.iterations, 'iterations', 1),
...
// Persist the --cli provider into config.json, preserving other keys.
function saveProvider(dir, provider) {
  const file = path.join(dir, 'config.json')
  let saved = {}
  if (fs.existsSync(file)) saved = JSON.parse(fs.readFileSync(file, 'utf8'))
  fs.writeFileSync(file, JSON.stringify({ ...saved, provider }, null, 2) + '\n')
}
...
function init(cwd) {
  ...
  console.log(`Initialized .atlas/ in ${cwd}\nAdd tasks to .atlas/backlog.md (or run 'atlas plan'), then run 'atlas'.`)
}
...
function status(cwd) {
  ...
  const branch = git.attempt(() => git.currentBranch(cwd))
...
async function plan(cwd, description, provider) {
...
function acquireLock(dir) {
  fs.mkdirSync(path.join(dir, 'runs'), { recursive: true })
  const file = path.join(dir, 'runs/runtime.lock')
  const token = randomUUID()
  const owner = git.attempt(() => JSON.parse(fs.readFileSync(file, 'utf8')))
  // A lock whose process is gone is stale and can be taken over.
  if (owner && git.attempt(() => process.kill(owner.pid, 0)) !== null) throw new Error(`Atlas is already running (PID ${owner.pid}). Remove ${file} only after it has stopped.`)
  fs.writeFileSync(file, JSON.stringify({ pid: process.pid, token }), { flag: owner ? 'w' : 'wx' })
  return () => {
    if (git.attempt(() => JSON.parse(fs.readFileSync(file, 'utf8')).token) === token) fs.unlinkSync(file)
  }
}
...
async function main() {
  ...
  const dir = path.join(cwd, '.atlas')
  if (options.command === 'init') init(cwd)
  if (!fs.existsSync(dir)) throw new Error(".atlas/ not found. Run 'atlas init' first.")
  if (options.cli && options.command !== 'status') saveProvider(dir, options.cli)
  ...
  try {
    if (options.command === 'plan') return await plan(cwd, options.args.join(' '), config.provider)
    return await loop.run(cwd, config, controller.signal)
  } finally {
    release()
  }
}
