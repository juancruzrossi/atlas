...
// Cap captured output so a runaway command can't grow it unbounded in memory.
...
function supervise(command, args, { cwd, log, timeout, signal, interactive = false }) {
  return new Promise((resolve, reject) => {
    ...
    // macOS reports EPERM for a group that only has zombies left.
    const kill = signalName => {
      try { process.kill(interactive ? child.pid : -child.pid, signalName) } catch (error) { if (!['ESRCH', 'EPERM'].includes(error.code)) throw error }
    }
    const stop = reason => {
...
module.exports = { PROVIDERS, hasBinary, runTask, runInteractive, runShell, interrupted }
